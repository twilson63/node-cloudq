...
var COMPLETE_UPDATE, DEQUEUE_UPDATE, QUEUE_VIEW, completeJob, db, dequeueJob, es, http, init, job, queueJob, relax, request, status, view;
...
queueJob = function(req, res) {
  ...
  return req.pipe(es.pipe(es.map(jobify), relax.post(db))).pipe(res);
};
