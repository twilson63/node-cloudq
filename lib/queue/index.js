...
app.router.post('/:queue', function(queue){
  var self = this;
  if(self.req.body.job) {
    request.post(cloudq, {json: jobify(self.req.body.job, queue) }, function(e,r,b){
      if (e) { 
        self.res.writeHead(500, { 'Content-Type': 'application/json'});
        self.res.end(e.message);
        console.log('RESPONSE: 500 -> ' + err.message);
      } else {
        self.res.writeHead(200, { 'Content-Type': 'application/json'});
        self.res.end(JSON.stringify(b));
        console.log('RESPONSE: 200 -> success');
      }
    });
  } else {
    console.log('RESPONSE: 500 -> ' + 'Job Object is required!');
